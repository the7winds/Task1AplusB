...
int main()
{
    ...
    OCL_SAFE_CALL(clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_ALL, devicesCount, deviceIds.data(), nullptr));
    ...
    cl_int err;
    cl_context context = clCreateContext(nullptr, 1, deviceIds.data(), nullptr, nullptr, &err);
    if (err != CL_SUCCESS) {
    ...
    // см. clBuildProgram
    clBuildProgram(program, 1, deviceIds.data(), nullptr, nullptr, nullptr);
    ...
    size_t log_size = 0;
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, deviceIds[0], CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size));
    std::vector<char> log(log_size, 0);
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, deviceIds[0], CL_PROGRAM_BUILD_LOG, log_size, log.data(), nullptr));
    if (log_size > 1) {
    ...
    {
        OCL_SAFE_CALL(clSetKernelArg(kernel, 0, sizeof(a), &a));
        OCL_SAFE_CALL(clSetKernelArg(kernel, 1, sizeof(b), &b));
        OCL_SAFE_CALL(clSetKernelArg(kernel, 2, sizeof(c), &c));
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            clEnqueueReadBuffer(queue, c, CL_TRUE, 0, 4 * n, cs.data(), 0, nullptr, nullptr);
            t.nextLap();
        }
